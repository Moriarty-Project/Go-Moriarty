# agh. This shall never see the light of day...
true = True
false = False
printString = """  {{"Name": "{0}","UrlHome": "{1}","UrlUsername": "{2}","UnclaimedIfResponseHas": "{3}","IsNSFW": {4}}},"""
foo = {
  "1337x": {
    "errorMsg": [
...
def main():
    ...
    for name in foo:
        item = foo[name]
        unclameResponse = "" if "errorMsg" not in item else item["errorMsg"]
        if len(unclameResponse)!=len(str(unclameResponse)):
            # we need to check it's not a multi line response... if it is, take the first...
            unclameResponse = unclameResponse[0]   
        unclameResponse = unclameResponse.replace('"','\\"')         
        nsfwTag = "false" if "isNSFW" not in item else item["isNSFW"]
        url = item['urlMain']
        testURL = str(item['url']).replace("{}","%s")
        text = printString.format(name, url, testURL, unclameResponse, nsfwTag)
        totalText+=text+"\n"
    print(totalText)
